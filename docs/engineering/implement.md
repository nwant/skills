...
You invoke this by typing `/implement` yourself: the agent won't reach for it on its own. It ships with `disable-model-invocation: true`, so no other skill can call it either. Wherever [which-skill](./which-skill.md) or [to-tickets](./to-tickets.md) says "then `/implement` per ticket", that is an instruction to you, not something the agent will do unprompted.
...
[which-skill](./which-skill.md) is the router over the whole set when you are not sure which flow you are in.
