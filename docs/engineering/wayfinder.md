...
The map and its tickets live on the repo's issue tracker, so wayfinder needs the tracker wiring that [setup-skills](./setup-skills.md) lays down. That step writes `docs/agents/issue-tracker.md`, including a "Wayfinding operations" section describing how the map, its child tickets, blocking edges, and frontier queries are expressed for GitHub, GitLab, or local markdown. Wayfinder reads that file by name. With no tracker configured it stops and tells you to run `/setup-skills` rather than falling back to local markdown: the map is the artifact the whole effort is kept in, and one charted on a tracker nobody chose is worse than none.
