...
> This file is maintained by hand. Everything from **1.2.3 down** is the release history of
> the upstream project, [mattpocock/skills](https://github.com/mattpocock/skills), preserved
> as written and generated by changesets, which this fork no longer uses. Skill names, issue
> numbers (`#NNN`) and PR references in those entries point at upstream, not at this fork.

## Fork

This repo diverged from [mattpocock/skills](https://github.com/mattpocock/skills) and is now
...
- `setup-matt-pocock-skills` is now `setup-skills` (`/setup-skills`)

Every call site moved with them: both README layers, the docs pages, and the six engineering
skills that name the setup skill as a precondition.

**One install route.** The Claude Code plugin path is gone: `.claude-plugin/`, the changeset
and version-sync machinery, the release workflow, and the npm manifests are all removed. The
plugin installed a managed, read-only bundle, which is the wrong shape for a repo you fork in
order to edit. `scripts/link-skills.sh` symlinks the working tree instead, so an edit is live
immediately and `git pull` updates every installed skill at once. Adding a skill no longer
requires a manifest entry, a changeset, or a version bump.

**Docs pages are repo-relative.** The 262 links that pointed at `aihero.dev/skills-<name>`, a
site this fork does not publish to, are now relative paths inside `docs/`. Each page gained an
H1, since that site was what generated titles from the slug. `.agents/writing-docs.md` and
`CLAUDE.md` carry the new convention so new pages follow it.
